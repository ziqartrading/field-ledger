(function(){
  ...
  const FL_BACKEND_SYNC_MIN_REV_R8=9;
  ...
  if(typeof flCheckBackendCompatibilityV381==='function'&&!flCheckBackendCompatibilityV381.__requiresR8){
    ...
    const flCheckBackendR8=async function(){
      ...
      if(revision<FL_BACKEND_SYNC_MIN_REV_R8)throw Object.assign(new Error(text('The tablet synchronization backend patch is missing or too old. Deploy backend tablet-sync-r9 or newer, then try again.','د ټابلیټ د سینک backend اصلاح نشته یا ډېره پخوانۍ ده. tablet-sync-r9 یا نوې نسخه خپره کړئ او بیا هڅه وکړئ.')),{code:'BACKEND_SYNC_PATCH_REQUIRED',status:status});
      return status;
    };
  ...
  const FL_FORM_TRANSPORT_PATCH_R9='2026-07-26-form-transport-session-r9';
